...
public class RecipeConditions {
	...
	public static Predicate<RecipeHolder<?>> isOfType(RecipeType<?>... otherTypes) {
		return recipe -> {
	...
	public static Predicate<RecipeHolder<?>> firstIngredientMatches(ItemStack stack) {
		return r -> !r.value().getIngredients().isEmpty() && r.value().getIngredients().get(0).test(stack);
	}

	public static Predicate<RecipeHolder<?>> outputMatchesFilter(FilteringBehaviour filtering) {
